...
sanitized = data %>%
    filter(! (Type == 'repeats' & Feature %in% c('rRNA', 'snRNA', 'tRNA'))) %>%
    filter(Feature != 'Other') %>%
    merge_features(DNA = c('DNA', 'DNA?')) %>%
    ...
    merge_features(`Protein coding` = c('IG_C_gene', 'IG_D_gene', 'IG_J_gene',
                                        ...
                                        'protein_coding')) %>%
    merge_features(lncRNA = c('3prime_overlapping_ncrna', 'antisense',
                              'lincRNA', 'processed_transcript',
                              'sense_intronic', 'sense_overlapping')) %>%
    merge_features(ncRNA = c('miRNA', 'misc_RNA', 'snoRNA', 'snRNA')) %>%
...
get_plot_data = function (data, criteria, top_features) {
    ...
    flevels = c(top_features, 'Other')
    remainder = used_data %>%
        filter(! Feature %in% top_features) %>%
        group_by(Stage) %>%
        summarize(TPM = sum(TPM)) %>%
        mutate(Feature = factor('Other', flevels))
    used_data %>%
        filter(Feature %in% top_features) %>%
