...
read_coverage_file = function (filename)
    ...
    select(Type = 4, Coverage = 5, BasesCovered = 6, BasesTotal = 7) %>%
    tbl_df()
```
...
info = as.data.frame(do.call(rbind, strsplit(all_files, '_'))) %>%
    select(DO = 1, IP = 2, Tissue = 3, Stage = 5, Feature = 7) %>%
    ...
    mutate(Feature = sub('\\.counts$', '', Feature)) %>%
